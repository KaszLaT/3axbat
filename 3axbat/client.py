import requests
import urllib3
from .utils import encrypt_password, get_device, sign_request, to_json
from .config import Config, Endpoints, Games
...
class Client:
    def __init__(self):
        ...
        self.engine_versions = Config.DEFAULT_ENGINE_VERSIONS
    ...
    def _request(self, method: str, path: str, params: dict = None, body: dict = None, auth: bool = True, use_dev: bool = False):
        ...
        if auth:
            headers.update({
                ...
                'manufacturer': Config.APP_TYPE,
                'package_name_en': Config.PACKAGE_NAME,
                'brand': Config.APP_TYPE,
                'model': '3AXBAT_PY',
            })
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        ak, nonce, timestamp, signature = sign_request(path, body=to_json(body), device=None)
        ...
        r = self.session.post(f'{self.base_url}{path}', headers=headers, json=body, timeout=15)
        if r.status_code == 200:
            data = r.json()
            if data.get('code') == 1:
    ...
    def get_profile(self, user_id: str = None) -> Profile:
        ...
        if user_id:
            path = Endpoints.OTHER_PROFILE.format(user_id=user_id)
            params = {'showBigParty': '1'}
            resp = self._request('GET', path, params=params)
        else:
            resp = self._request('GET', Endpoints.MY_PROFILE)
    ...
    def set_nickname(self, new_name: str) -> bool:
        ...
        resp = self._request('PUT', Endpoints.SET_NICKNAME, params={'newName': new_name, 'oldName': '3axbat'})
        if resp.get('code') == 1:
            return True
    ...
    def set_description(self, text: str) -> bool:
        ...
        resp = self._request('PUT', Endpoints.SET_USER_INFO, body={'details': text})
        if resp.get('code') == 1:
            return True
    ...
    def get_friends(self) -> list[Friend]:
        ...
        resp = self._request('GET', Endpoints.FRIENDS_STATUS, params={'showBigParty': '1'})
        if resp.get('code') == 1:
    ...
    def send_friend_request(self, user_id: int, game_type: int = None) -> bool:
        ...
        body = {
            'channel': 7, 
            'friendId': user_id, 
            'gameId': f'g{game_type}' if game_type else '', 
            'msg': '3axbat add', 
            'type': 1
        }
        resp = self._request('POST', Endpoints.ADD_FRIEND, body=body, use_dev=True)
        if resp.get('code') == 1:
            return True
    ...
    def get_game_server(self, game_type: int) -> str:
        ...
        eng1 = self.engine_versions.get('1', 10114)
        
        auth_resp = self._request('GET', Endpoints.GAME_AUTH, params={
            'typeId': type_id, 
            'userId': self.uid, 
            'targetId': self.uid, 
            'gameVersion': eng1
        ...
        path = Endpoints.GAME_DISPATCH
        ak, nonce, timestamp, signature = sign_request(path, body=bs, device=self.device)
        headers = {
            ...
            'x-urlpath': path, 'access-token': self.token,
            'Content-Type': 'application/json; charset=UTF-8', 'User-Agent': Config.USER_AGENT
        }
        
        r = self.session.post(f"{disp}{path}", headers=headers, data=bs, verify=False, timeout=15)
        if r.status_code == 200:
            resp = r.json()
            if resp.get('code') == 1:
